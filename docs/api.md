...
```json
{
  ...
  "incomeSchedule": [
    { "id": "btl_rental", "annualAmount": 9600, "fromYear": 0, "toYear": 15 }
  ],
  "expenseSchedule": [
...
**Debug-only field** (when `debug: true`):
- `resolvedYears` — array of per-year objects `{ income[], expense[], capitalOut[], capitalIn[], surplusOrder[], drawOrder[] }` as passed to the engine
